...
import { sanitizeTag, strArr, textOfContent, lastUserText, looksNatural, matchRitualWord, detectRitual, passesFrequency, isWorkTalk, shouldBoostRound, collectPrefsForEmotion, prefsScoreBonus, scoreStickers, substringMatch, genId, parseSuggestion, stripSuggestion, resolvePresetId, effectivePresetConfig } from './core.js'
...
export function apply(ctx) {
    ...
    const CHAT_TTL = 30 * 60 * 1000 // 和助手聊聊：会话 30 分钟无活动自动清理
    ...
    const observerState = { hint: null, lastByPreset: new Map() }
    // per-preset 运行时缓存：presetId -> { enabled, dialect:{id,boost}, freq:{daily,task} }
    // 方言 section 是同步渲染，读不了磁盘，靠 inbox/inserted 与 RPC 写入时预热
    const presetRuntimeCache = new Map()
    ...
    // ═══════════════ per-preset 配置（presets.json） ═══════════════
    // 全局 config.json 是默认值；presets.json 只存显式覆盖过的助手，字段缺省即继承全局。
    // 这样老用户不用迁移：没单独配置过的助手行为与旧版完全一致。
    async function loadPresetConfig() {
      const raw = await readJson('presets.json', null)
      if (!raw || typeof raw !== 'object' || !raw.presets || typeof raw.presets !== 'object') return { version: 1, presets: {} }
      return raw
    }
    async function savePresetConfig(data) { await writeJson('presets.json', data) }
    // 合并全局 + 覆盖 → 生效配置（并写进内存缓存供方言 section 同步渲染）
    async function resolvePresetRuntime(presetId) {
      try {
        const cfg = await loadConfig()
        const all = await loadPresetConfig()
        const override = all.presets[presetId] || null
        const eff = effectivePresetConfig(cfg, override)
        // 学我说话模板是全局一份（用户的说话风格），同步进方言配置供 section 读取
        if (eff.dialect && eff.dialect.id === 'userstyle') eff.dialect.template = dialectState.template
        presetRuntimeCache.set(presetId, eff)
        return eff
      } catch (e) {
        return presetRuntimeCache.get(presetId) || null
      }
    }
    // 扫描 dsh 助手预设（.agent-presets/），返回 [{ id, name }]
    async function listAgentPresets() {
      try {
        const home = await dshHome()
        if (!home) return []
        const root = home + '/.agent-presets'
        const rootTarget = await ctx.fs.resolve(root)
        const entries = await ctx.fs.listDir(rootTarget)
        const items = []
        for (const entry of (Array.isArray(entries) ? entries : [])) {
          if (!entry || entry.type !== 'directory' || !entry.name) continue
          const id = entry.name
          let name = id
          try {
            const metaText = await ctx.fs.readText(await ctx.fs.resolve(root + '/' + id + '/preset.yml'))
            const m = /(?:^|\n)\s*name\s*:\s*(.+?)\s*$/m.exec(String(metaText || ''))
            if (m) name = String(m[1]).replace(/^['"]|['"]$/g, '').trim() || id
          } catch (e) {}
          items.push({ id, name })
        }
    ...
    async function loadConfig() {
      ...
      if (typeof cfg.showFeedbackButtons !== 'boolean') cfg.showFeedbackButtons = true
      ...
      if (!cfg.freq || typeof cfg.freq !== 'object') cfg.freq = { version: 2, daily: 50, task: 20 }
      if (typeof cfg.freq.daily !== 'number') cfg.freq.daily = 50
      if (typeof cfg.freq.task !== 'number') cfg.freq.task = 20
      cfg.version = 4
      return cfg
    }
    ...
    // ═══════════════ 反馈与记录（per-preset：每个助手自己的偏好） ═══════════════
    // 查 sticker 在某情绪上下文下的反馈状态（按钮选中态 / 取消反馈用）
    // 返回 { state: 'positive'|'negative'|null, viaDislike, mappingIndex, dislikeCount }
    function getFeedbackState(prefs, stickerId, emotion, presetId) {
      const pid = presetId || 'default'
      const user = (prefs.users && prefs.users[pid]) || prefs.users.default
      const e = sanitizeTag(emotion, 60)
      const mappings = user.mappings
      const index = mappings.findIndex(m => m && m.context && m.context.emotion === e && !((m.context.keywords || []).length))
      if (index < 0) return { state: null }
      const m = mappings[index]
      if ((m.preferred_ids || []).includes(stickerId)) return { state: 'positive', viaDislike: false, mappingIndex: index }
      const dc = (m.dislike_counts || {})[stickerId] || 0
      if (dc > 0) return { state: 'negative', viaDislike: true, mappingIndex: index, dislikeCount: dc }
      if ((m.vetoed_ids || []).includes(stickerId)) return { state: 'negative', viaDislike: false, mappingIndex: index }
      return { state: null, mappingIndex: index }
    }
    async function applyFeedback(stickerId, emotion, kind, presetId) {
      const prefs = await loadPrefs()
      const pid = presetId || 'default'
      // 第一次在该助手下发反馈时创建独立偏好条目；之后该助手不再继承 default
      if (!prefs.users[pid]) prefs.users[pid] = { mappings: [] }
      const user = prefs.users[pid]
      const e = sanitizeTag(emotion, 60)
    ...
    // ═══════════════ AI 识图 / 内容分析 ═══════════════
    // 自定义 OpenAI 兼容端点调用（/chat/completions）：vision/content 模型都可配自定义 API
    async function callCustomLlm(custom, model, messages, maxTokens, temperature) {
      const baseUrl = String(custom && custom.baseUrl || '').trim().replace(/\/+$/, '')
      if (!baseUrl || !model) throw new Error('自定义 API 配置不完整')
      const headers = { 'Content-Type': 'application/json' }
      if (custom.apiKey) headers['Authorization'] = 'Bearer ' + String(custom.apiKey)
      const oaiMessages = (messages || []).map(m => ({
        role: m.role,
        content: (Array.isArray(m.content) ? m.content : [{ type: 'text', text: String(m.content || '') }]).map(p => {
          if (!p) return null
          if (p.type === 'text') return { type: 'text', text: String(p.text || '') }
          if (p.type === 'image') {
            const a = p.attachment || {}
            const b64 = a.bytes ? bytesToBase64(a.bytes) : ''
            return { type: 'image_url', image_url: { url: 'data:' + (a.mediaType || 'image/png') + ';base64,' + b64 } }
          }
          return null
        }).filter(Boolean)
      })).filter(m => m && m.content && m.content.length)
      ...
      let resp = null
      try {
        resp = await fetch(baseUrl + '/chat/completions', { method: 'POST', headers, body })
      } catch (e) {
        throw new Error('自定义 API 请求失败：' + (e && e.message ? e.message : String(e)))
      }
      if (!resp.ok) {
        let detail = ''
        try { const j = await resp.json(); detail = (j && j.error && j.error.message) ? j.error.message : JSON.stringify(j) } catch (e) {}
        throw new Error('自定义 API 返回 ' + resp.status + (detail ? '：' + String(detail).slice(0, 200) : ''))
      }
    ...
    async function resolveVisionTarget() {
      ...
      // 自定义 API 完整配置时优先
      const vc = config.visionCustom || {}
      if (vc.baseUrl && vc.apiKey && vc.model) return { provider: '__custom__', model: vc.model, custom: { baseUrl: vc.baseUrl, apiKey: vc.apiKey } }
    ...
    // 内容分析模型：聊天时自动配图（观察器）与「学我说话」风格分析用的独立模型。
    // 未配置时 fallback 识图模型 → 会话默认模型（零配置也能用，与旧版行为一致）
    async function resolveContentTarget() {
      const config = await loadConfig()
      const cc = config.contentCustom || {}
      if (cc.baseUrl && cc.apiKey && cc.model) return { provider: '__custom__', model: cc.model, custom: { baseUrl: cc.baseUrl, apiKey: cc.apiKey } }
      if (config.contentProvider && config.contentModel) return { provider: config.contentProvider, model: config.contentModel }
      return resolveVisionTarget()
    }
    async function callLlmText(messages, maxTokens, target) {
      const resolved = target || await resolveVisionTarget()
      if (!resolved) throw new Error('未配置模型')
      // 自定义 API 端点：直接走 /chat/completions
      if (resolved.custom) {
        return callCustomLlm(resolved.custom, resolved.model, messages, maxTokens || 1500, 0.3)
      }
      let text = ''
      let finish = null
      const stream = ctx.llm.stream({
        ...
        temperature: 0.3,
        // 显式 off：轻任务（识图/情绪分析/风格提炼）不需要思考；
        // 也避免继承 provider 默认推理级别（DeepSeek 直连对 flash 默认 max 会直接报错）
        reasoningEffort: 'off'
      })
    ...
    async function callLlmJson(messages, prompt, maxTokens, target) {
      const content = await callLlmText([{ role: 'user', content: [{ type: 'text', text: prompt }, ...messages] }], maxTokens || 800, target)
      let data = null
    ...
    async function runStyleAnalysis(level) {
      // 学我说话的风格提炼走内容分析模型（未配置则 fallback 识图/默认）
      const contentTarget = await resolveContentTarget()
      ...
      if (isDeep && collected.messages.length > 100) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          const s = await callLlmText([{ role: 'user', content: [{ type: 'text', text: buildChunkPrompt(chunkText, i + 1, chunks.length) }] }], 400, contentTarget)
          summaries.push(String(s).slice(0, 400))
        }
        const corpusText = buildCorpusText(sampled)
        const prompt = buildStylePrompt(corpusText + '\n\n【分块要点汇总】\n' + summaries.join('\n'), '')
        draft = await callLlmText([{ role: 'user', content: [{ type: 'text', text: prompt }] }], 1000, contentTarget)
      } else {
        const corpusText = buildCorpusText(sampled)
        const prompt = buildStylePrompt(corpusText, '')
        draft = await callLlmText([{ role: 'user', content: [{ type: 'text', text: prompt }] }], 1000, contentTarget)
    ...
    async function observerAnalyze(sessionId, text) {
      ...
      const contentTarget = await resolveContentTarget()
      const data = await callLlmJson(context, EMOTION_DETECT_PROMPT, 400, contentTarget)
    ...
    // ═══════════════ 系统提示注入：方言 / 学我说话（per-preset） ═══════════════
    // section.text 同步渲染时能拿到 context.agent.session，据此解析当前会话属于哪个助手，
    // 每个助手可以配自己的方言。缓存由 inbox/inserted 与 RPC 写入时预热。
    ...
    if (systemPrompt) {
      ctx.effect(() => systemPrompt.section({
        ...
        order: 60,
        text: (context) => {
          const session = context && context.agent ? context.agent.session : null
          const presetId = resolvePresetId(session)
          const cached = presetRuntimeCache.get(presetId)
          // per-preset 缓存优先；未预热时兜底全局 dialectState（与旧版行为一致）
          const dstate = (cached && cached.dialect) ? cached.dialect : dialectState
          const did = (dstate && dstate.id) || ''
          if (!did) return ''
          let body = ''
          if (did === 'userstyle') {
            body = (dstate && dstate.template) || ''
          } else {
            const d = DIALECTS[did]
    ...
    ctx.on('agent/inbox/inserted', async (payload) => {
      try {
        ...
        const sessionId = session ? String(session.header.id) : null
        const presetId = resolvePresetId(session)
        // 预热当前助手的运行时配置（方言 section 同步渲染依赖这个缓存）
        const eff = await resolvePresetRuntime(presetId)
        const freq = (eff && eff.freq) || { daily: 50, task: 20 }
        // 两阶段抽样：先按 max(daily, task) 预筛省模型调用，分析后再按场景概率校准。
        // 两档都为 0（日常/正事都不配图）时预筛永不通过 = 观察器自然关闭，无需独立开关
        if (!passesFrequency(Math.max(freq.daily || 0, freq.task || 0))) return
        const now = Date.now()
        const key = presetId || 'default'
        // 冷却按助手隔离，多会话并发不互相打断
        const lastTs = observerState.lastByPreset.get(key) || 0
        if (now - lastTs < 30000) return
        observerState.lastByPreset.set(key, now)
        const hit = await observerAnalyze(sessionId, text)
        if (hit && hit.emotion) {
          const sceneProb = hit.scene_type === '正事' ? (freq.task || 0) : (freq.daily || 0)
          if (!passesFrequency(sceneProb)) return
          observerState.hint = { emotion: hit.emotion, ts: now, sessionId, reason: hit.reason || '' }
          console.log('[biaoqingbao] 观察器: 检测到情绪 ' + hit.emotion + (hit.reason ? '（' + hit.reason + '）' : '') + ' preset=' + key)
        }
      } catch (e) {
        // 观察器是锦上添花，任何异常不影响聊天
      }
    ...
    ctx.on('agent/pre-step', async (payload, next) => {
      try {
        ...
        const sessionId = session ? String(session.header.id) : null
        const presetId = resolvePresetId(session)
        // 观察器提示注入（hint 带 session 核对，多会话并发不串台）
        if (observerState.hint && observerState.hint.ts > 0) {
          const hint = observerState.hint
          if (!hint.sessionId || hint.sessionId === sessionId) {
        ...
        // 方言加强版回响（按当前会话助手实时解析，不依赖缓存）
        let dstate = null
        const cached = presetRuntimeCache.get(presetId)
        if (cached && cached.dialect) dstate = cached.dialect
        if (!dstate) {
          const eff = await resolvePresetRuntime(presetId)
          if (eff && eff.dialect) dstate = eff.dialect
        }
        if (dstate && dstate.boost && dstate.id && dstate.id !== 'userstyle') {
          const d = DIALECTS[dstate.id]
          if (d && !isWorkTalk(lastUserText(messages)) && shouldBoostRound(messages.length)) {
            const echo = '你打字带着' + d.name + '味，这轮也照常。'
            const decision = await next()
            if (decision && decision.kind === 'enter') {
              // 注意：dsh 会话事件模型只有 user/message 与 assistant/message，role 必须是 'user'/'assistant'；
              // 注入的提示以 role 'user' + source.kind 'plugin' 标记（与 dsh-system-prompt 同姿势），
              // 用 'system' 会触发 SessionPersistenceCorruptionError: message must have role "user"
              return { kind: 'enter', messages: [...decision.messages, makePluginMessage(echo, 'user')] }
            }
            return decision
          }
        }
    ...
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        const sessionId = session ? String(session.header.id) : 'default'
        const presetId = resolvePresetId(session)
        // per-preset 开关：该助手没单独配过时继承全局默认
        const eff = await resolvePresetRuntime(presetId)
        if (!eff || eff.enabled === false) return { ok: false, error: '表情包功能已在「表情包」面板中关闭' }
        const stickers = await loadStickers()
        if (!stickers.length) return { ok: false, error: '表情包库是空的，请先在「表情包」面板上传一些表情包' }
        const prefs = await loadPrefs()
        // 每个助手自己的偏好；没单独调教过的助手继承 default
        const userPrefs = (prefs.users && prefs.users[presetId]) || prefs.users.default
        const prefsFor = collectPrefsForEmotion(userPrefs.mappings, emotion)
    ...
    ctx.tools.register(defineTool({
      ...
      async execute(args, exec) {
        ...
        const session = exec && exec.agent && exec.agent.session
        const presetId = resolvePresetId(session)
        const dislikeCount = await applyFeedback(id, emotion, 'negative', presetId)
    ...
    // RPC 没有 exec 上下文：优先 args.preset_id，其次 args.session_id（host 查事件解析），兜底 default
    async function presetIdFromArgs(args) {
      if (args && typeof args.preset_id === 'string' && args.preset_id) {
        const id = sanitizeTag(args.preset_id, 60)
        if (id) return id
      }
      if (args && typeof args.session_id === 'string' && args.session_id) {
        try {
          const sq = ctx.get('sessionQuery')
          if (sq && typeof sq.listEvents === 'function') {
            const events = await sq.listEvents(args.session_id)
            const pid = resolvePresetId({ header: null, events })
    ...
    rpc('feedback', async (args) => {
      ...
      const presetId = await presetIdFromArgs(args)
      const dislikeCount = await applyFeedback(id, emotion, kind, presetId)
      const prefs = await loadPrefs()
      const st = getFeedbackState(prefs, id, emotion, presetId)
      return { ok: true, dislike_count: dislikeCount, state: st.state, via_dislike: st.viaDislike === true, mapping_index: st.mappingIndex, preset_id: presetId }
    })

    rpc('feedback-state', async (args) => {
      const id = args && args.sticker_id ? String(args.sticker_id) : ''
      if (!id) return { ok: false, error: '缺少 sticker_id' }
      const emotion = sanitizeTag(args && args.emotion, 60)
      const presetId = await presetIdFromArgs(args)
      const prefs = await loadPrefs()
      const st = getFeedbackState(prefs, id, emotion, presetId)
      return { ok: true, data: st, preset_id: presetId }
    })
    ...
    rpc('stats', async () => {
      ...
      return {
        ok: true, data: {
          ...
          mappings: Object.values(prefs.users || {}).reduce((n, u) => n + ((u && u.mappings) || []).length, 0)
        }
      }
    })
    ...
    rpc('config-get', async () => {
      ...
      return {
        ok: true, data: {
          ...
          showFeedbackButtons: config.showFeedbackButtons !== false,
          visionProvider: config.visionProvider || '',
          visionModel: config.visionModel || '',
          contentProvider: config.contentProvider || '',
          contentModel: config.contentModel || '',
          visionCustom: (() => { const c = config.visionCustom || {}; return { baseUrl: c.baseUrl || '', apiKey: c.apiKey ? String(c.apiKey).substring(0, 8) + '***' : '', hasKey: !!c.apiKey, model: c.model || '' } })(),
          contentCustom: (() => { const c = config.contentCustom || {}; return { baseUrl: c.baseUrl || '', apiKey: c.apiKey ? String(c.apiKey).substring(0, 8) + '***' : '', hasKey: !!c.apiKey, model: c.model || '' } })(),
          observer: { enabled: false, frequency: 30 },
          freq: { daily: (config.freq && typeof config.freq.daily === 'number') ? config.freq.daily : 50, task: (config.freq && typeof config.freq.task === 'number') ? config.freq.task : 20 },
    ...
    rpc('config-set', async (args) => {
      ...
      if (args && typeof args.showFeedbackButtons === 'boolean') config.showFeedbackButtons = args.showFeedbackButtons
      ...
      if (args && typeof args.contentProvider === 'string') config.contentProvider = sanitizeTag(args.contentProvider, 100)
      if (args && typeof args.contentModel === 'string') config.contentModel = sanitizeTag(args.contentModel, 100)
      // 自定义 API 配置（OpenAI 兼容端点）；占位符 Key 不覆盖已存值
      for (const slot of ['vision', 'content']) {
        const field = slot + 'Custom'
        const input = args && args[field] && typeof args[field] === 'object' ? args[field] : null
        if (!input) continue
        const cur = config[field] || (config[field] = {})
        if (typeof input.baseUrl === 'string') cur.baseUrl = String(input.baseUrl).trim().slice(0, 300)
        if (typeof input.model === 'string') cur.model = String(input.model).trim().slice(0, 100)
        if (typeof input.apiKey === 'string' && input.apiKey.trim() && !String(input.apiKey).includes('***')) cur.apiKey = input.apiKey.trim().slice(0, 300)
      }
      ...
      if (args && args.freq && typeof args.freq === 'object') {
        const FREQ_VALUES = [0, 15, 50, 90]
        if (typeof args.freq.daily === 'number' && FREQ_VALUES.includes(args.freq.daily)) config.freq.daily = args.freq.daily
        if (typeof args.freq.task === 'number' && FREQ_VALUES.includes(args.freq.task)) config.freq.task = args.freq.task
      }
      ...
      // 全局默认值变了，清空 per-preset 缓存让下次解析重新合并
      presetRuntimeCache.clear()
      return { ok: true }
    })

    // ═══════════════ RPC：每位助手单独设置（per-preset） ═══════════════
    // 列表 = 扫 dsh 助手预设（.agent-presets/）+ 合并全局默认，返回每个助手当前生效配置
    rpc('presets-list', async () => {
      const presets = await listAgentPresets()
      const cfg = await loadConfig()
      const all = await loadPresetConfig()
      const items = presets.map(p => {
        const override = all.presets[p.id] || null
        const eff = effectivePresetConfig(cfg, override)
        return {
          id: p.id,
          name: p.name,
          ...
          enabled: eff.enabled,
          dialect: { id: (eff.dialect && eff.dialect.id) || '', boost: !!(eff.dialect && eff.dialect.boost) },
          freq: { daily: (eff.freq && typeof eff.freq.daily === 'number') ? eff.freq.daily : 50, task: (eff.freq && typeof eff.freq.task === 'number') ? eff.freq.task : 20 },
        }
      })
      return { ok: true, data: { items } }
    })

    // 写某助手的覆盖配置：只改传入字段，其余继承全局默认
    rpc('preset-set', async (args) => {
      const id = args && args.preset_id ? sanitizeTag(String(args.preset_id), 60) : ''
      if (!id) return { ok: false, error: '缺少 preset_id' }
      const all = await loadPresetConfig()
      let ov = all.presets[id] || {}
      if (args && typeof args.enabled === 'boolean') ov.enabled = args.enabled
      if (args && args.dialect && typeof args.dialect === 'object') {
        ov.dialect = ov.dialect || {}
        if (typeof args.dialect.id === 'string') {
          const did = args.dialect.id === '' ? '' : sanitizeTag(args.dialect.id, 30)
          ov.dialect.id = (did && !DIALECTS[did] && did !== 'userstyle') ? '' : did
        }
        if (typeof args.dialect.boost === 'boolean') ov.dialect.boost = args.dialect.boost
      }
      if (args && args.freq && typeof args.freq === 'object') {
        ov.freq = ov.freq || {}
        const FREQ_VALUES = [0, 15, 50, 90]
        if (typeof args.freq.daily === 'number' && FREQ_VALUES.includes(args.freq.daily)) ov.freq.daily = args.freq.daily
        if (typeof args.freq.task === 'number' && FREQ_VALUES.includes(args.freq.task)) ov.freq.task = args.freq.task
      }
      // 覆盖全部回到默认值时清理条目，避免积累空配置（空串方言 id 不算内容）
      const hasContent = (typeof ov.enabled === 'boolean')
        || (ov.dialect && ((ov.dialect.id && ov.dialect.id !== '') || typeof ov.dialect.boost === 'boolean'))
        || (ov.freq && (typeof ov.freq.daily === 'number' || typeof ov.freq.task === 'number'))
      if (!hasContent) delete all.presets[id]
      else all.presets[id] = ov
      await savePresetConfig(all)
      await resolvePresetRuntime(id)
      return { ok: true }
    })

    // 清除某助手的覆盖，回到全局默认
    rpc('preset-reset', async (args) => {
      const id = args && args.preset_id ? sanitizeTag(String(args.preset_id), 60) : ''
      if (!id) return { ok: false, error: '缺少 preset_id' }
      const all = await loadPresetConfig()
      delete all.presets[id]
      await savePresetConfig(all)
      await resolvePresetRuntime(id)
      return { ok: true }
    })
    ...
    const buildChatSystemPrompt = (sticker) => {
      const tags = sticker.tags || {}
      return '你是一个表情包标签整理助手，正在帮用户调教一张表情包的标签配得准不准。\n' +
        '当前表情包信息：\n' +
        '- id: ' + sticker.id + '\n' +
        '- 一句话描述: ' + (sticker.description || '（无）') + '\n' +
        '- 情绪标签: ' + ((tags.emotion || []).join('、') || '（无）') + '\n' +
        '- 场景标签: ' + ((tags.scene || []).join('、') || '（无）') + '\n' +
    ...
    rpc('chat', async (args) => {
      const stickerId = sanitizeTag(args && args.sticker_id, 40)
      const message = String(args && args.message || '').trim().slice(0, 1000)
      const sid = args && args.session_id ? String(args.session_id).slice(0, 64) : ''
      if (!stickerId || !message) return { ok: false, error: '缺少参数' }
      const stickers = await loadStickers()
      const sticker = stickers.find(s => s.id === stickerId)
      if (!sticker) return { ok: false, error: '未找到表情包' }
      // 会话获取/创建
      let session = null
      if (sid) session = chatSessions.get(sid)
      if (!session || session.stickerId !== stickerId) {
        session = { stickerId, messages: [], lastActive: Date.now() }
        session.sid = 'chat_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8)
        chatSessions.set(session.sid, session)
      }
      session.lastActive = Date.now()
      // 清理过期会话
      for (const [k, v] of chatSessions) { if (Date.now() - v.lastActive > CHAT_TTL) chatSessions.delete(k) }
      session.messages.push({ role: 'user', content: message })
      // 模型选择：优先当前会话默认模型（和「助手」聊），没有则用配置的识图模型兜底
      const config = await loadConfig()
      let provider = ''
      let model = ''
      try {
      ...
      if (!provider || !model) {
        provider = (config.visionProvider || '').trim()
        model = (config.visionModel || '').trim()
      }
      if (!provider || !model) return { ok: false, error: '未配置模型（请在设置页 AI 识图模型区块选择或填写）' }
      let text = ''
      let finish = null
      try {
        // dsh llm 层要求 content 为数组格式（[{ type: 'text', text }]），纯字符串会报 content.some is not a function
        // 且 assistant 历史消息必须带 source（dsh-llm forAdapter 会读 source.kind，缺了报 Cannot read properties of undefined (reading 'kind')）
        const fmtMsg = (m) => {
          const base = { role: m.role, content: [{ type: 'text', text: String(m.content || '') }] }
          if (m.role === 'assistant') base.source = { kind: 'model', provider, model }
          return base
        }
        const msgs = [fmtMsg({ role: 'system', content: buildChatSystemPrompt(sticker) }), ...session.messages.slice(-20).map(fmtMsg)]
        const stream = ctx.llm.stream({ provider, model, messages: msgs, maxTokens: 1000, temperature: 0.7, reasoningEffort: 'off' })
        for await (const chunk of stream) {
          if (chunk.type === 'text-delta') text += chunk.text
          else if (chunk.type === 'finish') finish = chunk.reason
        }
      } catch (e) {
        return { ok: false, error: '模型调用失败：' + (e && e.message ? e.message : String(e)) }
      }
      ...
      const cleanReply = stripSuggestion(text) || '（模型没有返回内容）'
      session.messages.push({ role: 'assistant', content: cleanReply })
      if (suggestion) session.suggestion = suggestion
      const tags = sticker.tags || {}
      return {
        ok: true,
        ...
        old_tags: {
          ...
          semantic_description: sticker.semantic_description || '',
          emotion: (tags.emotion || []).slice(),
          scene: (tags.scene || []).slice(),
          keywords: (tags.keywords || []).slice()
        }
      }
    })

    rpc('chat-apply', async (args) => {
      const sid = args && args.session_id ? String(args.session_id) : ''
      const session = chatSessions.get(sid)
      if (!session) return { ok: false, error: '会话不存在或已过期（重启后需重新聊）' }
      const sug = session.suggestion
      if (!sug) return { ok: false, error: '没有待应用的修改建议' }
      const stickers = await loadStickers()
      const sticker = stickers.find(s => s.id === session.stickerId)
      if (!sticker) return { ok: false, error: '未找到表情包' }
      if (sug.description) sticker.description = sanitizeTag(sug.description, 200)
      if (sug.semantic_description) sticker.semantic_description = sanitizeTag(sug.semantic_description, 500)
      if (!sticker.tags) sticker.tags = { emotion: [], scene: [], keywords: [] }
      if (sug.emotion) sticker.tags.emotion = strArr(sug.emotion)
      if (sug.scene) sticker.tags.scene = strArr(sug.scene)
      if (sug.keywords) sticker.tags.keywords = strArr(sug.keywords)
      sticker.updated_at = new Date().toISOString()
      await saveStickers(stickers)
    ...
    rpc('decision-log-list', async () => {
      const log = await readJson('decision-log.json', { version: 1, entries: [] })
      const entries = Array.isArray(log.entries) ? log.entries : []
      return { ok: true, data: entries.slice(-100).reverse() }
    })
    ...
    // ═══════════════ RPC：自定义 API 读取模型列表 ═══════════════
    rpc('custom-models', async (args) => {
      const slot = args && args.slot === 'content' ? 'content' : 'vision'
      let baseUrl = args && typeof args.baseUrl === 'string' ? String(args.baseUrl).trim().replace(/\/+$/, '') : ''
      let apiKey = args && typeof args.apiKey === 'string' ? String(args.apiKey).trim() : ''
      // 占位符 Key 时改用已保存的真实 Key
      if (apiKey.includes('***')) {
        try { const cfg = await loadConfig(); apiKey = ((cfg[slot + 'Custom'] || {}).apiKey) || '' } catch (e) { apiKey = '' }
      }
      ...
      try {
        ...
        if (!resp.ok) {
          let detail = ''
          try { const j = await resp.json(); detail = (j && j.error && j.error.message) ? j.error.message : '' } catch (e) {}
          return { ok: false, error: '读取失败（HTTP ' + resp.status + '）' + (detail ? '：' + String(detail).slice(0, 200) : '') }
        }
        const data = await resp.json()
        const list = Array.isArray(data && data.data) ? data.data : []
        const models = list.map(m => ({ id: String(m.id || ''), name: String(m.name || m.id || '') })).filter(m => m.id)
    ...
    // ═══════════════ RPC：识图/内容分析模型连通测试 ═══════════════
    // 优先级：表单当前值（custom → dsh 模型）→ 已保存配置（custom → dsh 模型）→ 未配置提示。
    // 用户不用先保存就能测刚选的模型（所见即所测）。
    rpc('vision-test', async (args) => {
      try {
        ...
        // 1. 表单当前值的自定义 API（占位符 Key 用已存值）
        const customInput = args && (slot === 'content' ? args.contentCustom : args.visionCustom)
        if (customInput && typeof customInput === 'object') {
          let b = String(customInput.baseUrl || '').trim().replace(/\/+$/, '')
          let k = String(customInput.apiKey || '').trim()
          let m = String(customInput.model || '').trim()
          if (k.includes('***')) {
            try { const cfg = await loadConfig(); k = ((cfg[slot + 'Custom'] || {}).apiKey) || '' } catch (e) { k = '' }
          }
          if (b && k && m) { customTarget = { baseUrl: b, apiKey: k }; model = m; provider = '__custom__' }
        }
        // 2. 表单当前值的 dsh 模型（下拉里选的）
        if (!customTarget && args && typeof args.visionProvider === 'string' && String(args.visionProvider).trim() && typeof args.visionModel === 'string' && String(args.visionModel).trim()) {
          provider = String(args.visionProvider).trim().slice(0, 100)
          model = String(args.visionModel).trim().slice(0, 100)
        }
        // 3. 已保存配置（没有表单输入时的兑底）
        if (!customTarget && !provider) {
          const config = await loadConfig()
          const c = config[slot + 'Custom'] || {}
          if (c.baseUrl && c.apiKey && c.model) {
            customTarget = { baseUrl: c.baseUrl, apiKey: c.apiKey }
            model = c.model
            provider = '__custom__'
          } else {
            provider = config[slot + 'Provider'] || ''
            model = config[slot + 'Model'] || ''
          }
        }
        if (customTarget) {
          const text = await callCustomLlm(customTarget, model, [{ role: 'user', content: [{ type: 'text', text: '连接测试，请只回复两个字：正常' }] }], 50, 0)
          return { ok: true, data: { provider: '自定义 API', model, reply: String(text || '').slice(0, 100) } }
        }
        if (!provider || !model) {
          return { ok: false, error: slot === 'content' ? '未配置内容分析模型（请在模型区块选择，或填写自定义 API）' : '未配置识图模型（请在模型区块选择，或填写自定义 API）' }
        }
        let text = ''
        let finish = null
        const stream = ctx.llm.stream({
