...
import { sanitizeTag, strArr, textOfContent, lastUserText, looksNatural, matchRitualWord, detectRitual, passesFrequency, isWorkTalk, shouldBoostRound, collectPrefsForEmotion, prefsScoreBonus, scoreStickers, substringMatch, genId, parseSuggestion, stripSuggestion } from './core.js'
...
export function apply(ctx) {
    ...
    function getFeedbackState(prefs, stickerId, emotion) {
    ...
    ctx.on('agent/inbox/inserted', async (payload) => {
      try {
        ...
        const freq = cfg.freq || { daily: 50, task: 20 }
    ...
    rpc('feedback', async (args) => {
      ...
      const prefs = await loadPrefs()
      const st = getFeedbackState(prefs, id, emotion)
      return { ok: true, dislike_count: dislikeCount, state: st.state, via_dislike: st.viaDislike === true, mapping_index: st.mappingIndex }
    })
    ...
    rpc('config-get', async () => {
      ...
      return {
        ok: true, data: {
          ...
          freq: { daily: (config.freq && config.freq.daily) || 50, task: (config.freq && config.freq.task) || 20 },
    ...
    rpc('chat', async (args) => {
      ...
      try {
        ...
        const stream = ctx.llm.stream({ provider, model, messages: msgs, maxTokens: 1000, temperature: 0.7 })
        for await (const chunk of stream) {
          if (chunk.type === 'text-delta') text += chunk.text
          else if (chunk.type === 'finish') finish = chunk.reason
        }
